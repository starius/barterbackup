...
impl TorTransport {
    ...
    pub async fn bind_peer_listener(&self, server_priv: &SecretKey) -> Result<TorPeerListener> {
        ...
        let onion_address = keys::onion_hostname_from_public_key(&ed25519_dalek::PublicKey::from(
            server_priv,
        ));
        let tls_acceptor =
...
impl PeerConnector for TorTransport {
    async fn connect(&self, peer_onion: &str, client_private_key: &SecretKey) -> Result<PeerClient> {
