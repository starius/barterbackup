...
struct Optimal {
    ...
    pi: Vec<HashMap<usize, Guess>>,
    /// same structure as optimal.m -- holds the overall metric.
    g: Vec<HashMap<usize, Guess>>,
}
...
lazy_static! {
    pub(crate) static ref REFERENCE_YEAR: i32 = {
        ...
        extern "C" {
            fn unix_time_milliseconds_imported() -> Guess;
        }
        ...
        chrono::DateTime::<chrono::Utc>::from(
            std::time::SystemTime::UNIX_EPOCH
                + std::time::Duration::from_millis(unix_millis as u64),
        )
        .year()
    };
}
...
const BRUTEFORCE_CARDINALITY: Guess = 10;
const MIN_GUESSES_BEFORE_GROWING_SEQUENCE: Guess = 10_000;
const MIN_SUBMATCH_GUESSES_SINGLE_CHAR: Guess = 10;
const MIN_SUBMATCH_GUESSES_MULTI_CHAR: Guess = 50;
...
pub fn most_guessable_match_sequence(
    ...
    fn update(
        ...
        let mut guesses = (factorial(len) as Guess).saturating_mul(pi);
        if !exclude_additive {
...
fn estimate_guesses(m: &mut Match, password: &str) -> Guess {
...
trait Estimator {
    fn estimate(&mut self, token: &str) -> Guess;
}
...
impl Estimator for DictionaryPattern {
    fn estimate(&mut self, token: &str) -> Guess {
        ...
        self.base_guesses = self.rank as Guess;
...
fn uppercase_variations(token: &str) -> Guess {
...
fn l33t_variations(pattern: &DictionaryPattern, token: &str) -> Guess {
    if !pattern.l33t {
        return 1;
    }
    ...
    for (subbed, unsubbed) in pattern.sub.as_ref().unwrap() {
        ...
        if subbed == 0 || unsubbed == 0 {
            ...
            let possibilities: Guess = (1..=p).map(|i| n_ck(unsubbed + subbed, i)).sum();
    ...
    fn estimate(&mut self, token: &str) -> Guess {
        ...
        if shifted_count > 0 {
            ...
            if unshifted_count == 0 {
                ...
                let shifted_variations: Guess = (1..=cmp::min(shifted_count, unshifted_count))
...
lazy_static! {
    static ref KEYBOARD_AVERAGE_DEGREE: Guess = calc_average_degree(&crate::adjacency_graphs::QWERTY);
    // slightly different for keypad/mac keypad, but close enough
    static ref KEYPAD_AVERAGE_DEGREE: Guess = calc_average_degree(&crate::adjacency_graphs::KEYPAD);
    static ref KEYBOARD_STARTING_POSITIONS: Guess = crate::adjacency_graphs::QWERTY.len() as Guess;
    static ref KEYPAD_STARTING_POSITIONS: Guess = crate::adjacency_graphs::KEYPAD.len() as Guess;
}

fn calc_average_degree(graph: &HashMap<char, Vec<Option<&'static str>>>) -> Guess {
    let sum: Guess = graph
        .values()
        .map(|neighbors| neighbors.iter().filter(|n| n.is_some()).count() as Guess)
        .sum();
    sum / graph.len() as Guess
}

impl Estimator for RepeatPattern {
    fn estimate(&mut self, _: &str) -> Guess {
        self.base_guesses.saturating_mul(self.repeat_count as Guess)
    }
}
...
impl Estimator for SequencePattern {
    fn estimate(&mut self, token: &str) -> Guess {
        ...
        base_guesses * token.chars().count() as Guess
    }
}
        ...
        if CHAR_CLASS_BASES.keys().any(|x| *x == self.regex_name) {
            ...
            match &*self.regex_name {
                "recent_year" => {
                    ...
                    cmp::max(year_space, MIN_YEAR_SPACE) as Guess
                }
                _ => unreachable!(),
            }
        }
...
lazy_static! {
    static ref CHAR_CLASS_BASES: HashMap<&'static str, Guess> = {
...
impl Estimator for DatePattern {
    fn estimate(&mut self, _: &str) -> Guess {
        ...
        let mut guesses = year_space as Guess * 365;
...
mod tests {
    ...
    fn test_repeat_guesses() {
        ...
        for &(token, base_token, repeat_count) in &test_data {
            ...
            let expected_guesses = base_guesses * repeat_count as Guess;
    ...
    fn test_regex_guesses_lowercase() {
        ...
        assert_eq!(p.estimate(token), Guess::from(26u64.pow(7)));
    }
    ...
    fn test_regex_guesses_alphanumeric() {
        ...
        assert_eq!(p.estimate(token), Guess::from(62u64.pow(5)));
    }
    ...
    fn test_regex_guesses_distant_year() {
        ...
        assert_eq!(
            p.estimate(token),
            (*scoring::REFERENCE_YEAR - 1972) as Guess
        );
    }
    ...
    fn test_regex_guesses_recent_year() {
        ...
        assert_eq!(p.estimate(&token), scoring::MIN_YEAR_SPACE as Guess);
    }
    ...
    fn test_date_guesses() {
        ...
        assert_eq!(
            p.estimate(token),
            365 * (*scoring::REFERENCE_YEAR - p.year).unsigned_abs() as Guess
        );
    }
    ...
    fn test_date_guesses_recent_years_assume_min_year_space() {
        ...
        assert_eq!(
            p.estimate(token),
            365 * scoring::MIN_YEAR_SPACE as Guess * 4
        );
    }
    ...
    fn test_spatial_guesses_accounts_for_turn_positions_directions_and_start_keys() {
        ...
        let token = "zxcft6yh";
        let guesses: Guess = (2..(token.len() + 1))
            .map(|i| {
