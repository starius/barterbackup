...
impl fmt::Display for CrackTimeSeconds {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let seconds = match self {
            ...
            CrackTimeSeconds::Float(f) => *f as Guess,
        };
        const MINUTE: Guess = 60;
        const HOUR: Guess = MINUTE * 60;
        const DAY: Guess = HOUR * 24;
        const MONTH: Guess = DAY * 31;
        const YEAR: Guess = MONTH * 12;
        const CENTURY: Guess = YEAR * 100;
        if seconds < 1 {
...
impl From<CrackTimeSeconds> for std::time::Duration {
    fn from(s: CrackTimeSeconds) -> std::time::Duration {
        match s {
            ...
            CrackTimeSeconds::Integer(i) => std::time::Duration::from_secs(i as u64),
        }
    }
}

pub(crate) fn estimate_attack_times(guesses: Guess) -> (CrackTimes, Score) {
    (CrackTimes::new(guesses), calculate_score(guesses))
}

fn calculate_score(guesses: Guess) -> Score {
    const DELTA: Guess = 5;
